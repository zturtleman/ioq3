...
Draws predicted effects for the shotgun, super shotgun and chaingun.  The
...
void CG_PredictWeaponEffects( centity_t *cent ) {
	...
	if ( ent->weapon == WP_SHOTGUN || ent->weapon == WP_SUPER_SHOTGUN ) {
		...
		if ( cg_delag.integer & 1 || cg_delag.integer & 4 ) {
			...
			if ( ent->weapon == WP_SHOTGUN ) {
				...
				CG_SuperShotgunPattern( muzzlePoint, endPoint, cg.oldTime % 256, cg.predictedPlayerState.clientNum );
			}
